...
IF NOT EXIST %UserProfile%\Desota\DeRunner mkdir %UserProfile%\Desota\DeRunner
...
IF %PROCESSOR_ARCHITECTURE%==x86 powershell -command "Invoke-WebRequest -Uri https://github.com/git-for-windows/git/releases/download/v2.41.0.windows.3/Git-2.41.0.3-32-bit.exe -OutFile ~\git_installer.exe" && start /B /WAIT %UserProfile%\git_installer.exe && del %UserProfile%\git_installer.exe
:skipinstallgit
