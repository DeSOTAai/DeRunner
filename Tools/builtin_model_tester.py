...
def call_model(model_req, result_file):
    ...
    if str(_model_runner_param["desota_runner"]).endswith(".py"):
    ...
    if str(_model_runner_param["desota_runner"]).endswith(".bat"):
        _model_runner = os.path.join(USER_PATH, _model_runner_param["desota_runner"])          # Model runner path
    #print(_model_runner_param)
    ...
    if _model_runner_py:
            ...
            if _ret_code != None:
                ...
                if _model_isTool:
                    try:
                        ...
                        _total_stdout = [f'Model stdout error: {e}', f'generated by desota']
                    delogger(f"[ DEBUG ] -> Process stdout: {json.dumps(_total_stdout, indent=2)}")
            break
                    # Check Timeout
        ...
        delogger(f"[ DEBUG ] -> Initialize Model Tested with output {_ret_code} (0=OK;>0=ERR)!...")
...
def main(args):
    ...
    try: # Run Test
        ...
        _model_id = model_req['task_model']
        _model_runner_param = SERV_PARAMS[_model_id][USER_SYS]
        print("*"*80)
        delogger("*"*80)
